...
OSStatus AudioLoopback_ioProc(AudioObjectID inDevice, const AudioTimeStamp *,
                              const AudioBufferList *inInputData,
                              const AudioTimeStamp *,
                              AudioBufferList *outOutputData,
                              const AudioTimeStamp *, void *inClientData) {
  AudioLoopback *self = static_cast<AudioLoopback *>(inClientData);
  return self->handleIOProc(inInputData, outOutputData);
}

AudioLoopback::AudioLoopback(AudioObjectID deviceID) : deviceID(deviceID) {}
...
bool AudioLoopback::start() {
  ...
  OSStatus status =
      AudioDeviceCreateIOProcID(deviceID, AudioLoopback_ioProc, this, &proc);
  if (status != noErr)
    return false;
  ioProcID = proc;
...
OSStatus AudioLoopback::handleIOProc(const AudioBufferList *inInputData,
                                     AudioBufferList *outOutputData) {
  if (inInputData == nullptr || outOutputData == nullptr)
    return kAudioHardwareNoError;

  const UInt32 numberInputBuffers =
      inInputData->mNumberBuffers > 0 ? inInputData->mNumberBuffers : 0;
  const UInt32 numberOutputBuffers =
      outOutputData->mNumberBuffers > 0 ? outOutputData->mNumberBuffers : 0;
